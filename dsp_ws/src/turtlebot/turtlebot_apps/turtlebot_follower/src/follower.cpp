...
private:
  ...
  virtual void onInit()
  {
    ...
    //pub3 = nd.advertise<std_msgs::String>("/voice/xf_asr_follow",3);
    sub_= nh.subscribe<sensor_msgs::Image>("depth/image_rect", 1, &TurtlebotFollower::imagecb, this);
    FLAGsub_=nh.subscribe<std_msgs::String>("/voice/xf_asr_follow",3,&TurtlebotFollower::FOLLOW_CB,this);
  ...
  /************************************************************/
 void FOLLOW_CB(const std_msgs::String::ConstPtr &msg)
{
	std::cout<<"get topic text : "<<msg->data.c_str();
	if (strcmp(msg->data.c_str(),"跟随开始")==0)
	  FLAG=1;
	if (strcmp(msg->data.c_str(),"跟随结束")==0)
	  FLAG=2;
//	else FLAG=0;	
	cout<<"FLAG="<<FLAG<<endl;
}

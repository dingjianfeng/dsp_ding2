...
private:
  ...
  virtual void onInit()
  {
    ...
    subFollow=nh.subscribe<std_msgs::Int32>("/following",3,&TurtlebotFollower::CallbackFollow,this);
  ...
  void CallbackFollow(const std_msgs::Int32::ConstPtr &msg)
{
  cout<<"get topic text : "<<msg->data<<endl;
  FOLLOW=msg->data;
...
//    cout<<"FOLLOW="<<FOLLOW<<endl;
 ...
 // add 5.29,too close and set stop     
      if(z <= goal_z_){
        ROS_INFO_THROTTLE(1, "Centroid too close %f, stopping the robot\n", z);
        if (enabled_)
        {
          cmdpub_.publish(geometry_msgs::TwistPtr(new geometry_msgs::Twist()));
        }

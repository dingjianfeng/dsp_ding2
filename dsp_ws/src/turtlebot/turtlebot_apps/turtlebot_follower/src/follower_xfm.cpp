...
/*
//add xfm model:turn to voice
void FOLLOW_XFM(const std_msgs::Int32::ConstPtr &msg)
{
	std::cout<<"get topic text : "<<msg->data;
	if (msg->data==1)
	  TurnToVoice=1;
	cout<<"TurnToVoice="<<TurnToVoice<<endl;
}
*/
  ...
  void imagecb(const sensor_msgs::ImageConstPtr& depth_msg)
  {
    ...
    if (n>4000)
    {
      ...
      if (enabled_)
      {
        ...
        ROS_INFO("speed is %d",cmd->linear.x);
        cmdpub_.publish(cmd);
 	TurnToVoice=0;
      }
    }
    else
    {
      ...
      if ((enabled_ ==true) && (TurnToVoice==0))
      {
        cmdpub_.publish(geometry_msgs::TwistPtr(new geometry_msgs::Twist()));
	ROS_INFO("TurnToVoice=%d",TurnToVoice);
        TurnToVoice=1;
      }
    }

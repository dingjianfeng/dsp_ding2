...
######
        ...
        if self.drink_type == '可乐':
            self.hmin = rospy.get_param("~hmin", 75)
            self.hmax = rospy.get_param("~hmax", 140)
            self.smin = rospy.get_param("~smin", 56)
            self.smax = rospy.get_param("~smax", 255)
            self.vmin = rospy.get_param("~vmin", 42)
            self.vmax = rospy.get_param("~vmax", 255)
            rospy.loginfo("匹配可乐")
        
        if self.drink_type == '雪碧':
            self.hmin = rospy.get_param("~hmin", 22)
            self.hmax = rospy.get_param("~hmax", 62)
            self.smin = rospy.get_param("~smin", 115)
            self.smax = rospy.get_param("~smax", 255)

...
class BaseController:
	...
	if msg.data == 'startcharge':
	   self.arduino.port.write('c 1.0 0 0 '+ '\r')
           rospy.loginfo("CMD:StartCharge...")
        if msg.data == 'stopcharge':
	   self.arduino.port.write('c 0 0 0 ' + '\r')
	   rospy.loginfo("CMD:StopCharge...")
    def AutoUpDownCallBack(self,msg):
	if msg.data == 'up':
	   self.arduino.port.write('l 1.0 0.0 0.0 ' + '\r') 
	   rospy.loginfo("CMD:Start UP...")  
	if msg.data == 'down':
	   self.arduino.port.write('l -1.0 0.0 0.0  ' + '\r')  
	if msg.data == 'stop':
	   self.arduino.port.write('l 0.0 0.0 0.0  ' + '\r') 
	   rospy.loginfo("CMD:Stop..")
class ArduinoROS():
    def __init__(self):
	...
	if msg.data == 'up':
	   self.controller.port.write('l 1.0 0 0 ' + '\r') 
	   rospy.loginfo("CMD:Start UP...")  
	if msg.data == 'down':
	   self.controller.port.write('l -1.0 0 0 ' + '\r')   

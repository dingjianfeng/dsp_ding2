...
###############################################################
    ...
    def __init__(self):
        ...
        locations['起始点'] = Pose(Point(-0.623, -1.517, 0.000), Quaternion(0.000, 0.000, 0.712, 0.703))
        locations['厨房'] = Pose(Point(-0.871, 0.108, 0.000), Quaternion(0.000, 0.000, -0.006, 1.000))
        locations['目标点'] = Pose(Point(0.525, -1.682, 0.000), Quaternion(0.000, 0.000, -0.670, 0.743))
        locations['客厅'] = Pose(Point(4.576, -0.226, 0.000), Quaternion(0.000, 0.000, -0.375, 0.927))
	...
	pub8 = rospy.Publisher('/dsp_arm',String,queue_size=5)     #arm by ding
...
#            location = locations.keys()[i]
	    location = dict_keys[i]
            ...
            rospy.loginfo("Going to 1: " + str(location))       
...
#                    pub1 = rospy.Publisher('/voice/xf_tts_topic', String, queue_size=5)
          	    ...
          	    ############# now next asr ######################	
                    ...
                    if str(location) == '厨房':
                    	...
                    	rospy.sleep(5)
                    	command="putup"
                    	pub3.publish(command)   #old
			pub8.publish(command)   #new by ding
			rospy.sleep(5)
                    	track=1
                    	pub4.publish(track)
                    	rospy.sleep(10) 
		        ...
		        rospy.loginfo("Dist2:"+str(self.Dist)) 
		        if self.Dist==0:
		        	rospy.loginfo("go away Distd1:"+str(self.Dist/1000.0+1.30))
		        	self.goal = MoveBaseGoal()
            			self.goal.target_pose.pose.position.x = self.Dist/1000.0+1.30
            			self.goal.target_pose.pose.orientation.w = 1.0; 
            			self.goal.target_pose.header.frame_id = 'base_footprint'
            			self.goal.target_pose.header.stamp = rospy.Time.now()            
		        	self.move_base.send_goal(self.goal)		        		        
		        	...
		        	rospy.loginfo("dd go away Distd2:"+str(self.Dist/1000.0-0.540))		    
   		       	
		        	self.goal = MoveBaseGoal()
            			self.goal.target_pose.pose.position.x = self.Dist/1000.0-0.540
            			self.goal.target_pose.pose.orientation.w = 1.0; 
            			self.goal.target_pose.header.frame_id = 'base_footprint'
            			self.goal.target_pose.header.stamp = rospy.Time.now()            
		        	self.move_base.send_goal(self.goal)		        		        
		        	rospy.sleep(5)
                    	...
                    	################################################                    	
                    	commandB1="grab"
                    	pub3.publish(commandB1)  #old
			pub8.publish(commandB1)  #new by ding
                    	rospy.sleep(10)
...
#                    	rospy.Subscriber('xf_xfm_node', Int32, self.callbackXFM)
			...
			commandC2="给您"+str(Drink)
                    	pub1.publish(commandC2)
                    	rospy.sleep(3)
                    	commandC1="putdown"
                    	pub3.publish(commandC1)   #old
			pub8.publish(commandC1)   #new by ding
			
			commandD1="home"
			pub8.publish(commandD1)   #new by ding
                    	rospy.sleep(50)
                    ##################################################      
                    if str(location) == '客厅':
#                    	rospy.sleep(5)
